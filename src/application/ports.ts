...
export interface QQGatewayPort {
	start(): Promise<void>;
	stop(): Promise<void>;
}
...
export interface ConfigRepositoryPort {
	load(): Promise<{ config: PiAgentQQBotConfig; missing?: boolean; parseError?: string }>;
	mutateAccess(change: AccessChange): Promise<PiAgentQQBotConfig>;
}
