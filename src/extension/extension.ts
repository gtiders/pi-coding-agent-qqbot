/**
 * pi-agent-qqbot extension entry.
 *
...
import type { PiAgentQQBotConfig } from "../application/ports";

const OWNER_TOKEN = Symbol("pi-agent-qqbot-extension-owner");
let currentConfig: PiAgentQQBotConfig | undefined;
let host: AgentQQBotHost | undefined;
...
export default function (pi: ExtensionAPI) {
	...
	const attachOwner = async (config: PiAgentQQBotConfig, ctx?: ExtensionContext): Promise<AgentQQBotHost> => {
		const acquired = await acquireAgentQQBotHost(config);
	...
	pi.registerCommand("qqbot-start", {
		...
		handler: async (_args, ctx) => {
			if (!currentConfig || !currentConfig.enabled) {
				ctx.ui.notify("pi-agent-qqbot: not enabled (set \"enabled\": true in ~/.pi/agent/pi-agent-qqbot.json)", "warning");
				return;
			}
			const invalid = validateEnabled(currentConfig);
			if (invalid) {
				ctx.ui.notify(`pi-agent-qqbot: cannot start (${invalid})`, "warning");
				return;
			}
			...
			if (!started) {
				disposeTerminalView();
				ctx.ui.notify("pi-agent-qqbot failed to start; use /qqbot-status for details", "error");
				return;
			}
			if (terminalView) runtime()?.attachObserver(terminalView);
			ctx.ui.notify(
				`${alreadyRunning ? "pi-agent-qqbot already running" : "pi-agent-qqbot started"}\n${viewAttached ? "conversation view: attached" : `conversation view: unavailable in ${ctx.mode} mode`}\n${runtime()?.statusText() ?? ""}`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("qqbot-stop", {
		...
		handler: async (_args, ctx) => {
			disposeTerminalView();
			if (!host?.getRuntime()) {
				ctx.ui.notify("pi-agent-qqbot is not running", "info");
				return;
			}
			await host.stop();
			ctx.ui.notify("pi-agent-qqbot stopped", "info");
		},
	});
	...
	pi.registerCommand("qqbot-status", {
		...
		handler: async (_args, ctx) => {
			ctx.ui.notify(`${runtime()?.statusText() ?? "pi-agent-qqbot is not running"}\nhost owners: ${host?.ownerCount ?? 0}`, "info");
		},
	});
	...
	pi.registerCommand("qqbot-runtime", {
		...
		handler: async (_args, ctx) => {
			...
			if (!diagnostics) {
				ctx.ui.notify("pi-agent-qqbot runtime is not initialized", "info");
				return;
			}
			const lines = [
	...
	pi.registerCommand("qqbot-reconnect", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.notify(runtime()?.statusText() ?? "pi-agent-qqbot is not running", "info");
		},
	});
	...
	pi.registerCommand("qqbot-requests", {
		...
		handler: async (_args, ctx) => {
			const rt = runtime();
			if (!rt) {
				ctx.ui.notify("pi-agent-qqbot is not running", "warning");
				return;
			}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (invalid) {
			if (ctx.hasUI) ctx.ui.notify(`pi-agent-qqbot: cannot start (${invalid})`, "warning");
			return;
		}
		...
		if (config.debug && !debugCommandRegistered) {
			...
			pi.registerCommand("qqbot-fake", {
				...
				handler: async (args, cctx) => {
					...
					if (!runtime()) {
						cctx.ui.notify("pi-agent-qqbot is not running", "warning");
						return;
					}
		...
		if (config.startup.mode === "auto" || restoreRuntime) {
			...
			if (!started && ctx.hasUI) ctx.ui.notify("pi-agent-qqbot: automatic startup failed", "error");
			if (started && restoreRuntime && ctx.hasUI) ctx.ui.notify("pi-agent-qqbot: runtime replaced and QQ gateway restored", "info");
		} else if (config.startup.mode === "manual" && !runtime()?.isReady() && ctx.hasUI) {
			ctx.ui.notify("pi-agent-qqbot: ready (manual mode). Use /qqbot-start to connect.", "info");
		}
	});
